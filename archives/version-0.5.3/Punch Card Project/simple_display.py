...
DEFAULT_WELCOME_MESSAGE = "WELCOME TO PUNCH CARD DISPLAY"
...
def save_message_to_database(message, source="local"):
    ...
    if not db_connection:
        debug_log("Database not initialized - message not saved", "warning")
        return False
    
    try:
        # Just log for testing
        debug_log(f"Message saved to database: '{message}' (source: {source})", "info")
        return True
    except Exception as e:
        debug_log(f"Error saving message: {e}", "error")
        return False
    ...
    if message:
        ...
        if hasattr(display, 'display_message'):
            try:
                ...
                # Set up clear timer if not already done
                display_time = int(config.get("message_display_time", 5) * 1000)  # Convert to milliseconds
                if hasattr(display, 'clear_timer') and display.clear_timer.isActive():
                    display.clear_timer.stop()  # Stop existing timer
                
                # Create or update clear timer
                if not hasattr(display, 'clear_timer'):
                    from PyQt6.QtCore import QTimer
                    display.clear_timer = QTimer()
                    display.clear_timer.timeout.connect(clear_display_message)
                
                # Start timer to clear message after specified time
                display.clear_timer.start(display_time)
                debug_log(f"Message will be cleared after {display_time/1000} seconds", "info")
            except Exception as e:
...
class SettingsDialog(QDialog):
    ...
    def _setup_general_tab(self):
        ...
        self.display_time_spin.setRange(1, 60)  # 1 to 60 seconds
        ...
        display_layout.addRow("Display Time:", self.display_time_spin)
    ...
    def load_from_settings(self):
        ...
        display_time = config.get("message_display_time", 5)
        self.display_time_spin.setValue(display_time)
    ...
    def get_settings(self):
        ...
        settings["message_display_time"] = self.display_time_spin.value()
...
def initialize_database():
    ...
    try:
        # For our testing, just set this to a placeholder
        db_connection = True
        debug_log("Database initialized (placeholder)", "info")
        return True
    except Exception as e:
        debug_log(f"Error initializing database: {e}", "error")
        return False
...
def clear_display_message():
    """Clear the currently displayed message."""
    global display
    
    if not display:
        debug_log("Cannot clear message - display object not initialized", "error")
        return
    
    # Try to clear message using appropriate method
    try:
        if hasattr(display, 'clear_message'):
            display.clear_message()
            debug_log("Message cleared using clear_message()", "info")
        elif hasattr(display, 'punch_card') and hasattr(display.punch_card, 'clear_grid'):
            display.punch_card.clear_grid()
            debug_log("Message cleared using punch_card.clear_grid()", "info")
        elif hasattr(display, 'clear_grid'):
            display.clear_grid()
            debug_log("Message cleared using clear_grid()", "info")
        else:
            debug_log("No suitable method found to clear the message", "warning")
    except Exception as e:
        debug_log(f"Error clearing message: {e}", "error")

def initialize_display(app):
    ...
    try:
        ...
        # Create a simple display widget for testing
        class SimpleDisplay(QMainWindow):
            ...
            def display_message(self, message):
                ...
                debug_log(f"Displayed message: {message}")
                
            def clear_message(self):
                """Clear the currently displayed message."""
                self.message_label.setText("")
                debug_log("Message cleared")
        
        # Create display instance
        display = SimpleDisplay()
        debug_log("Created SimpleDisplay instance")
        ...
        if display:
            ...
            debug_log("Failed to create display object", "error")
            return None
    except Exception as e:
        debug_log(f"Error initializing display: {str(e)}", "error")
        return None
...
# Main function - runs when the script is executed directly
if __name__ == "__main__":
    # Parse command line arguments
    import argparse
    parser = argparse.ArgumentParser(description="Punch Card Display")
    parser.add_argument("--debug", action="store_true", help="Enable debug mode")
    parser.add_argument("--message", type=str, help="Initial message to display")
    args = parser.parse_args()
    ...
    try:
        ...
        display = initialize_display(app)
        
        if not display:
            update_api_console("❌ Failed to initialize display", "error")
            sys.exit(1)
        
        # Show initial message or welcome
        try:
            welcome_message = args.message if args.message else DEFAULT_WELCOME_MESSAGE
            
            if hasattr(display, 'display_message'):
                display.display_message(welcome_message)
                
                # Set up message clear timer
                display_time = int(config.get("message_display_time", 5) * 1000)  # Convert to milliseconds
                if not hasattr(display, 'clear_timer'):
                    from PyQt6.QtCore import QTimer
                    display.clear_timer = QTimer()
                    display.clear_timer.timeout.connect(clear_display_message)
                display.clear_timer.start(display_time)
                debug_log(f"Initial message will be cleared after {display_time/1000} seconds", "info")
                
                # Save to database if enabled
                if hasattr(display, 'message_db') and config.get("save_to_database", True):
                    display.message_db.save_message(welcome_message, "local")
                    debug_log("Welcome message saved to database")
            else:
                debug_log("Display does not have display_message method", "warning")
        except Exception as e:
        ...
        debug_log(f"Error in main application: {e}", "error")
        sys.exit(1) 