...
A PyQt6 widget for displaying punch card messages with exact IBM specifications.
"""
...
CARD_HEIGHT = int(82.55 * SCALE_FACTOR)

class PunchCardWidget(QWidget):
    """A widget that displays a punch card with exact IBM specifications."""
    ...
    def paintEvent(self, event):
        ...
        for row in range(self.num_rows + 1):
            y = int((row * CARD_HEIGHT) / self.num_rows)  # Convert to int
            painter.drawLine(0, y, CARD_WIDTH, y)
        
        for col in range(self.num_cols + 1):
            x = int((col * CARD_WIDTH) / self.num_cols)  # Convert to int
    ...
    def _draw_hole(self, painter, row, col):
        ...
        x = int((col * CARD_WIDTH) / self.num_cols + (CARD_WIDTH / self.num_cols) / 2)  # Convert to int
        y = int((row * CARD_HEIGHT) / self.num_rows + (CARD_HEIGHT / self.num_rows) / 2)  # Convert to int
        radius = int(min(CARD_WIDTH, CARD_HEIGHT) / (max(self.num_rows, self.num_cols) * 2))  # Convert to int
