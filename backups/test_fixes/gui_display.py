...
try:
    from src.utils.message_bus import (
        get_message_bus, EVENT_NEW_MESSAGE, EVENT_DISPLAY_COMPLETE,
        EVENT_PUNCH_CARD_UPDATED, EVENT_API_STATUS_CHANGED
    )
    ...
    def init_api_manager(self):
        ...
        try:
            ...
            if success:
                ...
                if self.message_bus:
                    # Import the constant from global scope if needed
                    from src.utils.message_bus import EVENT_API_STATUS_CHANGED
...
class PunchCardDisplay(QMainWindow):
    ...
    def start_splash_screen(self):
        ...
        if hasattr(self, 'console'):
            self.console.log("Splash screen started", "INFO")
        
        # Start the startup animation if animation manager is available
        if self.animation_manager:
            self.console.log("Using animation manager for splash screen", "INFO")
            self.animation_manager.play_animation(AnimationType.STARTUP, callback=self.complete_splash_screen)
        else:
            # If animation manager is not available, use simple splash animation
            self.console.log("Animation manager not available, using simple splash animation", "WARNING")
    ...
    def start_animation(self):
        ...
        if hasattr(self, 'animation_manager') and self.animation_manager:
            self.console.log("Using animation manager for splash animation", "INFO")
            self.animation_manager.play_animation(AnimationType.STARTUP, callback=self.complete_splash_screen)
        else:
            # If animation manager is not available, use simple splash animation
            self.console.log("Animation manager not available, using simple splash animation", "WARNING")
...
def run_gui_app():
    ...
    try:
        ...
        punch_card = PunchCardWidget()  # Use PunchCardWidget instead of PunchCard
